...
function HomeButton({ isActive }: { isActive?: boolean }) {
  const color = isActive ? "#fff" : "#649fa9";

  return (
    <button
      className={`block p-2 cursor-pointer  rounded-full   ${
        isActive && "bg-green-500 button-shadow--active"
      }`}
    >
      <svg
...
export default function ActionBar(props: ActionBarProps) {
  return (
    ...
    >
      <ul className="flex justify-evenly items-center w-full h-full -mr-[3px] bg-[#236e78] rounded-l-full max-w-xs">
        <li className="">
          <HomeButton isActive />
        </li>{" "}
        <li className="">
