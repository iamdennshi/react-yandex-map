...
export default function SearchBox({places, setCurrentPlace} : Searchbox) {
    ...
    const searchboxItems = places.filter((i) => i.address.includes(fromInput)).map((i) => <li key={i.id} tabIndex={i.id} onClick={onItemClick} className='searchbox-item px-4 py-2  hover:bg-green-50'>{i.address}</li>);

  return (
    <>
    <div onClick={() => setIsActive(false)} className={`fixed transition-all duration-500 ${isActive ? " opacity-100 " : " opacity-0 invisible"} z-20 inset-0  bg-black bg-opacity-50`}></div>
    <div onClick={hideOverlay}  className='absolute z-20 left-1/2 top-4 -translate-x-2/4 max-w-xl w-full px-4 '>
        <input ref={inputRef} value={fromInput} className={`block w-full px-5 py-2 pl-12 rounded-full border-solid border-2 ${isActive ? "border-green-500" : "border-[#ccc]"}  outline-none shadow-lg text-[#4A5568] font-medium transition`} type="text" placeholder='Название объекта' onChange={({target}) => setFromInput(target.value)} />
